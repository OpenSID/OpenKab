...
<script nonce="{{  csp_nonce() }}">
    document.addEventListener("DOMContentLoaded", function (event) {
        let articles = $('#articles-table').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: false,
            ordering: true,
            ...
            ajax: {
                ...
                data: function (row) {
                    return {
                        ...
                        "sort": (row.order[0]?.dir === "asc" ? "" : "-") + row.columns[row.order[
                                0]?.column]?.name,
                    };
                },
        ...
        articles.on('draw.dt', function () {
            var PageInfo = $('#articles-table').DataTable().page.info();
            articles.column(0, {
                page: 'current'
