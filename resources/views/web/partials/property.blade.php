    ...
    $.get('{{ url('index.php/api/v1/desa-aktif') }}', {}, function(result){
        if (result.data.length > 0){
            ...
            result.data.forEach((item, index) => {
                ...
                _elm.find('.penduduk-elm').text(item.attributes.penduduk+ ' Penduduk')
                ...
                _elm.find('.keluarga-elm').text(item.attributes.keluarga+ ' Keluarga')
                _elm.find('.rtm-elm').text(item.attributes.rtm+ ' RTM')
            })
        }
    }, 'json')
});
